...
## Git hygiene when pairing manual `n8n backup` with the scheduled backup
...
runs on the laptop write the same files. Every backup — scheduled or manual —
...
`n8n status` / `n8n backup` with `json.decoder.JSONDecodeError: Expecting

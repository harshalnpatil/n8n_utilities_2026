...
def build_minimal_credential_data(schema: Dict[str, Any], cred_type: str) -> Dict[str, Any]:
    """Build minimal credential data object that satisfies schema."""
    data = {}

    # Handle OAuth2-like schemas more robustly
    ...
    if all_of:
        ...
        for sub_schema in all_of:
            ...
            all_required.update(sub_schema.get("required", []))
    ...
    # Credential type-specific overrides for complex schemas
    type_specific_fields = {
        "gmailOAuth2": {
            "serverUrl": "https://accounts.google.com/o/oauth2/auth",
            "clientId": "__placeholder_clientId__",
            "clientSecret": "__placeholder_clientSecret__",
        ...
        "zohoOAuth2Api": {
            "serverUrl": "https://accounts.zoho.com/oauth/v2/auth",
            "clientId": "__placeholder_clientId__",
            "clientSecret": "__placeholder_clientSecret__",
            "sendAdditionalBodyProperties": False,
            "additionalBodyProperties": {},
        ...
        "openAiApi": {
            ...
            "headerValue": "Bearer __placeholder_api_key__",
        },
        "aws": {
            "accessKeyId": "__placeholder_accessKeyId__",
            "secretAccessKey": "__placeholder_secretAccessKey__",
            "sessionToken": "__placeholder_sessionToken__",
            ...
            "ssmEndpoint": "https://ssm.us-east-1.amazonaws.com",
            "rekognitionEndpoint": "https://rekognition.us-east-1.amazonaws.com",
        },
    }

    # Generic OAuth2 fields for any OAuth2-like credential
    generic_oauth_fields = {
        "serverUrl": "https://accounts.google.com/o/oauth2/auth",
        "clientId": "__placeholder_clientId__",
        "clientSecret": "__placeholder_clientSecret__",
        "sendAdditionalBodyProperties": False,
    ...
    # Check if this credential type has specific overrides
    override_fields = type_specific_fields.get(cred_type, {})

    for prop_name, prop_schema in properties.items():
        # Check if this is a field we have specific handling for
        if prop_name in override_fields:
            data[prop_name] = override_fields[prop_name]
        elif prop_name in generic_oauth_fields and prop_name in required:
            data[prop_name] = generic_oauth_fields[prop_name]
        elif prop_name in required or not all_of:  # Basic schema handling
            ...
            if default is not None:
                ...
                if enum_values:
                    data[prop_name] = enum_values[0]  # Use first enum value
                else:
...
def main():
    ...
    print('=' * 70)
    print('CREDENTIAL MIGRATION UTILITY')
    print('=' * 70)
    print('')
    print('[!] IMPORTANT: This script reads from LOCAL workflow JSON files.')
    print('    Make sure your local workflow files are up to date by running:')
    print("    python scripts/n8n_sync.py --mode backup --instance [source-instance]")
    print('')
    print('    This ensures all credential references are captured correctly.')
    print('')
    ...
    for ref in sorted(all_refs, key=lambda r: (r.cred_type, r.cred_name)):
        ...
        try:
            data = build_minimal_credential_data(schema, ref.cred_type)
